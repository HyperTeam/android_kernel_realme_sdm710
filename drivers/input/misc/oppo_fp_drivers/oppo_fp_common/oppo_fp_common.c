...
static int fp_gpio_parse_parent_dts(struct fp_data *fp_data)
{
    ...
    // Shengwang.Luo@BSP.Fingerprint.Basic 2019-10-12 for fingerprint id gpio
    if (fp_data->fp_id_amount == 3) {
        int fp_gpio = of_get_named_gpio(np, FP_GPIO_NODE, 0);
        if (ret < 0) {
            dev_err(dev, "failed to get '%s'\n", FP_GPIO_NODE);
            return FP_ERROR_GPIO;
      	}
        ret = devm_gpio_request(dev, fp_gpio, FP_GPIO_NODE);
        if (ret) {
            dev_err(dev, "failed to request gpio %d\n", fp_gpio);
            devm_gpio_free(dev, fp_gpio);
            return FP_ERROR_GPIO;
        }
        dev_info(fp_data->dev, "fp-id gpio: %d\n", fp_gpio);
        fp_data->fp_id_amount = 1;
        fp_data->gpio_index[0] = fp_gpio;
        fp_data->fp_id[0] = gpio_get_value(fp_data->gpio_index[0]);
        dev_info(fp_data->dev, "gpio_index: %d,fp_id: %d\n", fp_data->gpio_index[0], fp_data->fp_id[0]);
	} else {
